...
void Schema::initialize()
{
    for (auto &e : enums)
    {
        if (e.getName() != "EObjectType")
            continue;
        auto &items = e.getItems();
        executeForClasses([&](const auto &cls)
        {
            for (auto &c : cls)
            {
                auto i = std::find_if(items.begin(), items.end(), [&c](const auto &i) { return i.name == c.getCppName(); });
                if (i == items.end())
                {
                    EnumItem ei;
                    ei.name = c.getCppName();
                    items.push_back(ei);
                }
            }
        });
...
ModuleEmitter Schema::printObjectInterfaces() const
{
    ModuleEmitter mc;
    ...
    executeForClasses([&](const auto &cls)
    {
        for (auto &c : cls)
        {
            ...
            mc.hpp.addLineWithoutIndent("#ifndef POLYGON4_" + nameUpper + "_INTERFACE");
            mc.hpp.addLineWithoutIndent("DECLARE_INTERFACE_STUB(" + name + ");");
            mc.hpp.addLineWithoutIndent("#endif");
            mc.hpp.addLine();
        }
    });
    mc.hpp.endNamespace();
...
ModuleEmitter Schema::printForwardDeclarations() const
{
    ModuleEmitter mc;
    mc.hpp.addLine(autogenerated_warning);
    mc.hpp.addLine();
    ...
    mc.hpp.addLine();
    mc.hpp.endNamespace("detail");
    mc.hpp.endNamespace("polygon4");
    return mc;
}

ModuleEmitter Schema::printTypes() const
{
    ModuleEmitter mc;
    mc.hpp.addLine(autogenerated_warning);
    mc.hpp.addLine();
    ...
    executeForClasses([&](const auto &cls)
    {
        for (auto &c : cls)
        {
            mc.hpp.addLine("#include \"types/" + c.getName() + ".h\"");
            mc.cpp.addLine("#include \"types/" + c.getName() + ".cpp\"");
        }
    });
    return mc;
}

ModuleEmitter Schema::printType(const Class &c) const
{
    ModuleEmitter mc;
    mc.hpp.addLine(autogenerated_warning);
    mc.hpp.addLine();
...
ModuleEmitter Schema::printEnums() const
{
    ModuleEmitter mc;
    mc.hpp.addLine(autogenerated_warning);
    mc.hpp.addLine();
    ...
    mc.hpp.endNamespace("detail");
    for (auto &e : enums)
        mc.hpp.addLine(dll + "\nString tr(detail::" + e.getCppName() + " e);");
    mc.hpp.addLine();
    for (auto &e : enums)
        mc.hpp.addLine(dll + "\nString str(detail::" + e.getCppName() + " e);");
    mc.hpp.addLine();
    mc.hpp.endNamespace("polygon4");

    mc.cpp.addLine(autogenerated_warning);
    mc.cpp.addLine();
    mc.cpp.beginNamespace("polygon4");
    mc.cpp.beginNamespace("detail");
    for (auto &e : enums)
        mc += e.printTableRecord();

    // getOrderedMap()
    {
        mc.cpp.beginFunction("EnumTable getOrderedMap(const std::string &type_name)");
        mc.cpp.addLine("EnumTable et;");
        mc.cpp.addLine("std::string ending;");
        for (auto &e : enums)
        {
            mc.cpp.addLine("ending = \"" + e.getEnding() + "\";");
            mc.cpp.addLine("CREATE_TABLE(" + e.getCppName() + ");");
        }
        mc.cpp.addLine("return et;");
        mc.cpp.endFunction();
    }

    mc.cpp.endNamespace("detail");
    auto print_enum_function = [&mc](const auto &e, const std::string &fname, const std::string &fcall)
    {
        mc.cpp.beginFunction("::polygon4::String " + fname + "(detail::" + e.getCppName() + " e)");
        mc.cpp.addLine("auto i = detail::" + e.getTableName() + ".find(e);");
        mc.cpp.addLine("if (i != detail::" + e.getTableName() + ".end())");
        mc.cpp.increaseIndent();
        mc.cpp.addLine("return " + fcall + "(i->second);");
        mc.cpp.decreaseIndent();
        mc.cpp.addLine("return MISSING_VALUE;");
        mc.cpp.endFunction();
    };
    for (auto &e : enums)
        print_enum_function(e, "tr", "tr");
    for (auto &e : enums)
        print_enum_function(e, "str", "");
    mc.cpp.endNamespace("polygon4");

    return mc;
}

ModuleEmitter Schema::printStorage() const
{
    const auto &cls = getClasses();
    const auto &cls_service = getServiceDbClasses();
    ...
    mc.hpp.addLineWithoutIndent("public:");
    for (auto &c : cls)
    ...
    mc.hpp.addLine();
    mc.hpp.addLine("// use before save into empty database");
    mc.hpp.addLine("virtual void create(Database &) const = 0;");
    mc.hpp.addLine("virtual void load(Database &, ProgressCallback) = 0;");
    mc.hpp.addLine("virtual void save(Database &, ProgressCallback) const = 0;");
    mc.hpp.addLine("// use before new load");
    ...
    executeForClasses([&mc](const auto &cls)
    {
        for (auto &c : cls)
    ...
    mc.cpp.beginFunction("Storage::~Storage()");
    mc.cpp.endFunction();
    mc.cpp.beginFunction("int Storage::get_storage_version() const");
    mc.cpp.addLine("return " + version + ";");
    mc.cpp.endFunction();
    mc.cpp.beginFunction("int Storage::get_db_version() const");
    mc.cpp.addLine("return db_version;");
    mc.cpp.endFunction();
    mc.cpp.beginFunction("std::string Storage::getSql()");
    mc.cpp.addLine("std::string sql;");
    executeForClasses([&mc](const auto &cls)
    {
        for (auto &c : cls)
            mc.cpp.addLine("sql += " + c.getCppName() + "::getSql();");
    });
...
ModuleEmitter Schema::printStorageImplementation() const
{
    const auto &cls = getClasses();
    const auto &cls_service = getServiceDbClasses();
    ...
    // create()
    {
        mc.hpp.addLine("void create(Database &) const override;");

        mc.cpp.beginFunction("void " + storageImpl + "::create(Database &db) const");
        executeForClasses([&mc](const auto &cls)
        {
            for (auto &c : cls)
    ...
    // clear()
    {
        mc.hpp.addLine("void clear() override;");
    ...
    // load()
    {
        double step = 100.0 / ((cls.size() + cls_service.size()) * 3.0);
        double progress = 0;

        mc.hpp.addLine("void load(Database &, ProgressCallback) override;");

        mc.cpp.beginFunction("void " + storageImpl + "::load(Database &db, ProgressCallback callback)");
        executeForClasses([&mc, &progress, &step](const auto &cls)
        {
            for (auto &c : cls)
            {
                mc.cpp.addLine("_load" + c.getSqlName() + "(db);");
    ...
    // save()
    {
        ...
        mc.hpp.addLine("void save(Database &, ProgressCallback) const override;");

        mc.cpp.beginFunction("void " + storageImpl + "::save(Database &db, ProgressCallback callback) const");
        for (auto &c : cls)
        {
            mc.cpp.addLine("_save" + c.getSqlName() + "(db);");
            mc.cpp.addLine("PROGRESS_CALLBACK(" + std::to_string(progress += step) + ");");
        }
        mc.cpp.endFunction();
    }
    ...
    // printAddDeleteRecord()
    {
        ...
        // addRecord()
        {
            mc.hpp.addLine("" + dataClassPtr + "<" + iObject + "> addRecord(" + iObject + " *parent = nullptr) override;");
        ...
        // deleteRecord()
        {
            mc.hpp.addLine("void deleteRecord(" + iObject + " *data) override;");
    ...
    // printTree()
    {
        ...
        // printTree()
        {
            mc.hpp.addLine("Ptr<TreeItem> printTree() const override;");
        ...
        // addRecord()
        {
            mc.hpp.addLine("Ptr<TreeItem> addRecord(TreeItem *item) override;");
        ...
        // deleteRecord()
        {
            mc.hpp.addLine("bool deleteRecord(TreeItem *item) override;");
            ...
            for (auto &c : cls)
            {
                ...
                mc.cpp.beginBlock("if (*i == item->object)");
    ...
    {
        mc.hpp.addLine();
        mc.hpp.addLine("OrderedObjectMap getOrderedMap(" + objectType + " type, CheckFunction f = CheckFunction()) const override;");

        auto prnt = [&](bool f)
        {
            mc.cpp.beginBlock("switch (type)", false);
            executeForClasses([&mc, f](const auto &cls)
            {
                for (auto &c : cls)
                {
                    mc.cpp.addLine("case " + objectType + "::" + c.getCppName() + ":");
...
ModuleEmitter Class::print(const Schema &schema) const
{
    ModuleEmitter mc;
    ...
    // class, data
    {
        ...
        mc.hpp.addLine("using Base = I" + getCppName() + ";");
        mc.hpp.addLine();
        mc.hpp.addLine("// data");
        mc.hpp.addLineWithoutIndent("public:");
        printVariables(mc);
    ...
    // constructors
    {
        ...
        for (auto &v : vars_container)
        {
            if (!v.hasFlags({ fArray }))
                continue;
            mc.cpp.addLine(v.getName() + ".setMemberPtr(&" + v.getType()->getCppName() + "::" + v.getArrayKey()->getName() + ");");
        }
        mc.cpp.endFunction();
        ...
        mc.cpp.addLine("Base::operator=(rhs);");
        mc.cpp.addLine("copyFrom(rhs);");
    ...
    if (hasIdField)
    {
        mc.hpp.addLine("bool hasId() const override;");
    ...
    // getType()
    {
        mc.hpp.addLine("" + objectType + " getType() const override;");
    ...
    // getVariableString()
    {
        mc.hpp.addLine("" + string_type + " getVariableString(int columnId) const override;");
    ...
    // setVariableString()
    {
        mc.hpp.addLine("void setVariableString(int columnId, const " + string_type + " &text) override;");

        mc.cpp.beginFunction("void " + getCppName() + "::setVariableString(int columnId, const " + string_type + " &text)");
        bool has_var = false;
        Emitter local;
        local.beginBlock("switch (columnId)", false);
        for (auto &v : vars)
        {
            if (v.isFk())
                continue;
            has_var = true;
            local.addLine("case " + std::to_string(v.getId()) + ":");
            local.increaseIndent();
            local.addLine("this->" + v.printSet() + ";");
            local.addLine("break;");
            local.decreaseIndent();
        }
        local.addLine("default:");
        local.increaseIndent();
        local.addLine("break;");
        local.endBlock(true);
        if (has_var)
    ...
    if (hasFks)
    {
        mc.hpp.addLine("void setVariableString(int columnId, " + iObject + " *ptr) override;");
        ...
        for (auto &v : vars)
        {
            ...
            local.addLine("case " + std::to_string(v.getId()) + ":");
            local.increaseIndent();
            local.addLine(v.printSetPtr() + ";");
            for (auto &d : v.getSlaveVariables())
                local.addLine(d->getName() + ".clear();");
            local.addLine("break;");
            local.decreaseIndent();
        }
        local.addLine("default:");
    ...
    if (hasFks)
    {
        mc.hpp.addLine("" + iObject + " *getVariable(int columnId) const override;");
        ...
        for (auto &v : vars)
        {
            ...
            local.addLine("case " + std::to_string(v.getId()) + ":");
            local.increaseIndent();
            local.addLine("return " + v.getName() + ".ptr;");
            local.decreaseIndent();
        }
        local.endBlock(true);
        if (has_var)
    ...
    if (hasFks)
    {
        mc.hpp.addLine("" + objectType + " getVariableType(int columnId) const override;");
        ...
        for (auto &v : vars)
        {
            ...
            local.addLine("case " + std::to_string(v.getId()) + ":");
            local.increaseIndent();
            local.addLine("return " + objectType + "::" + v.getType()->getCppName() + ";");
            local.decreaseIndent();
        }
        local.endBlock(true);
        if (has_var)
    ...
    // getName()
    {
        mc.hpp.addLine("" + string_type + " getName() const override;");
    ...
    if (vars.has("text_id"))
    {
        mc.hpp.addLine("" + string_type + " getTextId() const override;");
    ...
    {
        mc.hpp.addLine("" + string_type + " getDescription() const override;");
    ...
    // getClass()
    {
        mc.hpp.addLine("const Class &getClass() const override;");
    ...
    {
        mc.hpp.emptyLines(1);
        mc.hpp.addLine("std::tuple<bool, OrderedObjectMap> getOrderedObjectMap(int columnId, Storage *storage = nullptr) const override;");
        ...
        for (auto &v : vars)
        {
            ...
            {
                local.addLine("return std::make_tuple(true, " + v.getGetOrderedObjectMap() + ");");
            }
            else if (v.getType()->getCppName() == StringDbType)
            {
                local.addLine("if (!storage)");
                local.increaseIndent();
                local.addLine(fail);
                local.decreaseIndent();
                local.addLine("return std::make_tuple(true, storage->strings.getOrderedObjectMap([](const String *s)");
                local.addLine("{");
                local.increaseIndent();
                auto type = getCppName();
                if (!v.getEnumTypeName().empty())
                    type = v.getEnumTypeName();
                local.addLine("bool r = s->object == " + objectType + "::" + type + ";");
                local.addLine("r |= s->object == " + objectType + "::Any;");
                local.addLine("return r;");
                local.decreaseIndent();
                local.addLine("}));");
            }
            else
            {
                ...
                local.addLine(fail);
                local.decreaseIndent();
                local.addLine("return std::make_tuple(true, storage->" + v.getType()->getCppArrayVariableName() + ".getOrderedObjectMap());");
            }
            local.decreaseIndent();
        }
        local.endBlock(true);
    ...
    // deepCopyFrom()
    {
        mc.hpp.addLine("void deepCopyFrom(const " + getCppName() + " &rhs);");

        mc.cpp.beginFunction("void " + getCppName() + "::deepCopyFrom(const " + getCppName() + " &rhs)");
        for (auto &v : vars)
        {
            if (v.isId())
                continue;
        ...
        {
            mc.cpp.addLine();
            mc.cpp.addLine("auto s = getStorage();");
            mc.cpp.addLine();
            for (auto &v : vars_container)
            {
                mc.cpp.addLine(v.getPrefixedName() + " = rhs." + v.getPrefixedName() + ";");
                ...
                if (vclass->getFlags()[fProxy])
                {
                    ...
                    mc.cpp.beginBlock("for (auto &v : " + v.getPrefixedName() + ")");
                    mc.cpp.addLine("auto v2 = s->" + vclass->getCppArrayVariableName() + ".createAtEnd(*v);");
                    mc.cpp.addLine("v2->" + var_name + " = this;");
                    mc.cpp.addLine("v = v2;");
                    mc.cpp.endBlock();
                    mc.cpp.addLine();
                }
    ...
    // getSql()
    {
        ...
        mc.cpp.decreaseIndent();
        mc.cpp.addLine(sql);
        if (getCppName() == dbSetting)
        {
            mc.cpp.addLine();
            mc.cpp.addLine("insert into " + getSqlName() + " (key, value_int) values ('" + db_version + "', '" + schema.getVersion() + "');");
        }
        mc.cpp.addLine();
        mc.cpp.increaseIndent();
        mc.cpp.addLine(")sql\";");
...
ModuleEmitter Class::printIo() const
{
    ModuleEmitter mc;
    const auto vars = getVariables();
    ...
    // printLoad()
    {
        mc.hpp.addLine("void _load" + getSqlName() + "(Database &);");

        mc.cpp.beginFunction("void " + storageImpl + "::_load" + getSqlName() + "(Database &db)");
        mc.cpp.addLine("LOG_TRACE(logger, \"Loading " + getSqlName() + "\");");
        mc.cpp.addLine("const std::string query = \"select ");
        for (auto &v : vars)
        {
            mc.cpp.addText(v.getSqlName() + ", ");
        }
        mc.cpp.trimEnd(2);
        ...
        {
            mc.cpp.addLine();
            mc.cpp.addLine("auto map = " + getCppArrayVariableName() + ".getOrderedObjectMap();");
            mc.cpp.addLine("auto i = map.find(\"VERSION\");");
            mc.cpp.beginBlock("if (i != map.end())");
            mc.cpp.addLine("db_version = ((" + dbSetting + "*)i->second)->value_int;");
            mc.cpp.endBlock();
            mc.cpp.beginBlock("else");
            mc.cpp.addLine("db_version = 0;");
            mc.cpp.endBlock();
        }
        mc.cpp.endFunction();
    }
    ...
    // printSave()
    {
        mc.hpp.addLine("void _save" + getSqlName() + "(Database &) const;");

        mc.cpp.beginFunction("void " + storageImpl + "::_save" + getSqlName() + "(Database &db) const");
        if (!hasFlags({ fNoSave }))
        {
            mc.cpp.addLine("db.execute(\"BEGIN;\");");
            mc.cpp.addLine("db.execute(\"delete from " + getSqlName() + ";\");");
            mc.cpp.addLine("const std::string query = \"insert into " + getSqlName() + " (");
            for (auto &v : vars)
                mc.cpp.addText(v.getSqlName() + ", ");
            mc.cpp.trimEnd(2);
            mc.cpp.addText(") values (");
            for (auto &v : vars)
                mc.cpp.addText("?, ");
            mc.cpp.trimEnd(2);
            mc.cpp.addText(");\";");
...
ModuleEmitter Class::printAddDeleteRecordVirtual() const
{
    std::string param;
...
ModuleEmitter Class::printAddDeleteRecord() const
{
    const auto vars = getVariables();
    const auto vars_inline = vars({ fInline });
    ...
    // add record
    {
        mc.hpp.addLine("" + dataClassPtr + "<" + getCppName() + "> add" + getCppName() + "(" + param_h + ") override;");
    ...
    // delete record
    {
        mc.hpp.addLine("void delete" + getCppName() + "(" + iObject + " *object) override;");
...
void Class::printVariables(ModuleEmitter &mc) const
{
...
ModuleEmitter Enum::print() const
{
    ModuleEmitter mc;
...
ModuleEmitter Enum::printTableRecord() const
{
    ModuleEmitter mc;
...
std::string Variable::print() const
{
    ...
    if (flags[fContainer])
    {
        s += "CVector<" + type->getCppName() + "> " + getName();
        return s;
    }
    if (isFk())
    {
...
void Variable::printLoadSqlite3(Emitter &ctx, const std::string &var) const
{
    auto t = getDataType();
...
void Variable::printSaveSqlite3(Emitter &ctx, const std::string &var) const
{
    auto t = getDataType();
