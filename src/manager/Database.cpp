...
sqlite3 *load_from_file(const path &fn)
{
    ...
    {
        std::string error = "Can't open database file: " + to_printable_string(fn) + " error: " + sqlite3_errmsg(db);
        LOG_ERROR(logger, error);
        throw SW_RUNTIME_ERROR(error);
    }
    auto ret = loadOrSaveDb(db, (const char *)to_path_string(fn).c_str(), 0);
    if (ret != SQLITE_OK)
    {
        std::string error = "Can't load database: " + to_printable_string(fn) + " error: " + sqlite3_errstr(ret);
        LOG_ERROR(logger, error);
        throw SW_RUNTIME_ERROR(error);
    }
    return db;
...
{
    auto ret = loadOrSaveDb(db, (const char *)to_path_string(fn).c_str(), 1);
    if (ret != SQLITE_OK)
    {
        std::string error = "Can't save database: " + to_printable_string(fn) + " error: " + sqlite3_errstr(ret);
        LOG_ERROR(logger, error);
        throw SW_RUNTIME_ERROR(error);
    }
}
...
Database::Database(const path &dbname)
{
    ...
    loadDatabase(dbname);
    name = to_printable_string(dbname.filename());
    fullName = dbname;
}
